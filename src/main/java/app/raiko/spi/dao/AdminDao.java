...
public class AdminDao {
    ...
    try (var connection = DatabaseConfiguration.getConnection(config)) {
      ...
      try (var findAdminStatement =
          connection.prepareStatement(
              adminQuery, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE)) {
        ...
        if (existAdmin) admin = getAdminObject(findAdminResult);
      }
  ...
  private String getPasswordByUsernameQuery() {
    return String.format(
        ...
        Admin.PASSWORD_COLUMN, Admin.TABLE_NAME, Admin.USERNAME_COLUMN);
  }
