...
public class RoleDao {
  ...
  public Set<Role> findAdminRolesByAdminId(long adminId) {
    var roleQuery =
        getFindRolesQuery(
            Role.AdminRole.TABLE_NAME,
            Role.AdminRole.ROLE_ID_COLUMN,
            Role.AdminRole.ADMIN_ID_COLUMN);
  ...
  private Set<Role> getRoles(String roleQuery, long id) {
    ...
    try (var connection = DatabaseConfiguration.getConnection(config)) {
      try (var findRoleStatement =
          connection.prepareStatement(
              roleQuery, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE)) {
        ...
        while (rolesResult.next()) {
          ...
          var role = Role.builder().id(roleId).name(roleName).build();
          roles.add(role);
        }
      }
  ...
  public Set<Role> findUserRolesByUserId(long userId) {
    var roleQuery =
        getFindRolesQuery(
            Role.UserRole.TABLE_NAME, Role.UserRole.ROLE_ID_COLUMN, Role.UserRole.ADMIN_ID_COLUMN);
  ...
  public Set<Role> findVetRolesByVetId(long vetId) {
    var roleQuery =
        getFindRolesQuery(
            Role.VetRole.TABLE_NAME, Role.VetRole.ROLE_ID_COLUMN, Role.VetRole.ADMIN_ID_COLUMN);
